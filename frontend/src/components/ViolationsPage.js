  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${detectionBaseUrl}/detect-traffic-violation`,
        formData,
        ...
        <Grid item xs={12} md={6}>
          <Card sx={{ borderRadius: 3, height: '100%' }}>
            <CardContent sx={{ p: 4 }}>
              ...
              {analysisResult ? (
                <Box>
                  <Alert severity={analysisResult.traffic_violation_detected ? 'warning' : 'success'} sx={{ mb: 3 }}>
                    {analysisResult.traffic_violation_detected ? '⚠️' : '✅'} Traffic Violation Result
                  </Alert>

                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                    <Chip label={`Detected: ${analysisResult.traffic_violation_detected ? 'Yes' : 'No'}`} color={analysisResult.traffic_violation_detected ? 'warning' : 'success'} />
                    ...
                    <Grid container spacing={2}>
                      {analysisResult.saved_images.map((url) => (
                        <Grid item xs={12} sm={6} key={url}>
                          <Paper sx={{ p: 1 }}>
                            <img src={url} alt="traffic violation" style={{ width: '100%', borderRadius: 6 }} />
