  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${detectionBaseUrl}/detect-helmet`,
        formData,
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Upload a traffic video to detect helmet violations
        </Typography>
      </Box>
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              <Box sx={{ 
                ...
                <input
                  ...
                  style={{ display: 'none' }}
                  id="helmet-video-upload"
                />
                <label htmlFor="helmet-video-upload" style={{ cursor: 'pointer' }}>
        ...
        <Grid item xs={12} md={6}>
          <Card sx={{ borderRadius: 3, height: '100%' }}>
            <CardContent sx={{ p: 4 }}>
              ...
              {analyzing && (
                <Box sx={{ mb: 3 }}>
                  <Typography variant="body2" gutterBottom>Analyzing helmet violations...</Typography>
                  <LinearProgress />
                </Box>
              )}
              ...
              {analysisResult ? (
                <Box>
                  <Alert severity={analysisResult.helmet_violation_detected ? 'warning' : 'success'} sx={{ mb: 3 }}>
                    {analysisResult.helmet_violation_detected ? '⚠️' : '✅'} Helmet Violation Result
                  </Alert>

                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                    <Chip label={`Detected: ${analysisResult.helmet_violation_detected ? 'Yes' : 'No'}`} color={analysisResult.helmet_violation_detected ? 'warning' : 'success'} />
                    ...
                    <Grid container spacing={2}>
                      {analysisResult.saved_images.map((url) => (
                        <Grid item xs={12} sm={6} key={url}>
                          <Paper sx={{ p: 1 }}>
                            <img src={url} alt="helmet violation" style={{ width: '100%', borderRadius: 6 }} />
...
export default HelmetViolationPage;
